...
- [x] **H1** — Promote `describePath` from `coverageReport.ts` (private) to `paths.ts` (exported); extract `cli.ts`'s trace-directory-listing snippet into a shared, exported `listTraceFiles` helper (`coverageReport.ts`), now throwing `PathKitError` like the rest of the codebase's I/O helpers instead of only being catchable inline in `cli.ts`.
